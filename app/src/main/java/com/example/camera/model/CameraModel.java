...
public class CameraModel implements CameraContract.Model {
    ...
    private void drawStatistics(Canvas canvas, Bitmap originalBitmap, int avgR, int avgG, int avgB,
        ...
        CalibrationFactor absFactor = calibrationRepository != null
                ? calibrationRepository.loadCalibrationFactor()
                : null;
        double[] debevecG = calibrationRepository != null
                ? calibrationRepository.loadDebevecG()
                : null;
        boolean useDebevecAbs = absFactor != null && absFactor.isValid()
                && debevecG != null && debevecG.length >= 256;
        boolean hasGOnly = !useDebevecAbs && debevecG != null && debevecG.length >= 256;
        ...
        if (useDebevecAbs) {
            ...
            curveFormulaLine = "曲线: 未加载 Debevec 响应 g(DN)";
            curveBvLine = "请使用应用内 Debevec 曝光序列生成 g";
        }
    ...
    private static String formatDebevecAbsoluteSourceLine(CalibrationFactor factor) {
        ...
        if (factor.calibrationTimestamp > 0L) {
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd", Locale.CHINA);
            return String.format(Locale.CHINA, "来源: Debevec 标定 (%s)",
                    sdf.format(new Date(factor.calibrationTimestamp)));
        }
        return "来源: Debevec+绝对校准";
    }
    ...
    public double computeLFromDn(double dn) {
        if (calibrationRepository == null || !calibrationRepository.hasDebevecG()) {
            return Double.NaN;
        }
        CalibrationFactor factor = calibrationRepository.loadCalibrationFactor();
        if (factor == null || !factor.isValid()) {
            return Double.NaN;
        }
        double[] g = calibrationRepository.loadDebevecG();
        if (g == null || g.length < 256) {
            return Double.NaN;
        }
        int z = (int) Math.round(Math.max(0.0, Math.min(255.0, dn)));
        double lnE = g[z];
        double eRel = Math.exp(lnE);
        double L = AbsoluteLuminanceCalibration.convertToRealLuminance(eRel, factor);
        return Double.isFinite(L) ? L : Double.NaN;
    }

    /** 云端同步等完成后刷新 UI 文案（无内存曲线状态）。 */
    ...
    /** 预览角标：与 {@link #getCurveSourceLabel()} 一致简短版。 */
