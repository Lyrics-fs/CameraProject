...
class DebugStatsDashboardActivity : AppCompatActivity() {
    ...
    private fun formatCurrentCurve(): String {
        val p = CalibrationRepository(this).loadCurveParams()
        val df = DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.SHORT)
        val t = if (p.updatedAt > 0L) df.format(p.updatedAt) else getString(R.string.debug_stats_time_none)
        return when (p.source) {
            CurveParams.Source.PRIOR ->
                getString(
                    R.string.debug_stats_curve_prior,
                    p.a,
                    p.b,
                    p.rSquared,
                    p.fitSampleCount,
            ...
            CurveParams.Source.CALIBRATED ->
                getString(
                    R.string.debug_stats_curve_calibrated,
                    p.a,
                    p.b,
                    p.rSquared,
                    p.fitSampleCount,
            ...
            CurveParams.Source.CLOUD ->
                getString(
                    R.string.debug_stats_curve_cloud,
                    p.a,
                    p.b,
                    p.cloudProfileVersion,
                    p.cloudAggregatedDeviceCount,
                    p.cloudConfidence,
                    t,
                )
        }
