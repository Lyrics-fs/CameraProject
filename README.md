...
- 界面布局：预览区约占屏上 2/3；左上为 E 值与状态角标；左下为半透明叠层（模式切换 +「高级参数」2×2 + 其下的可折叠标定行）。下方面板为推荐/曲线/手动滑杆；底栏为拍照与推荐相关主操作。
...
6. 设备标定：在预览左下角、高级参数下方点「标定 ▾」展开，再依次「开始标定」→ 对准灰卡/色卡中心 → 多次「采样」→「完成拟合」。标定成功后可自动收起该行，需要时再点「标定 ▾」展开。
